...
def do_train(args, model, train_dataloader, save_dir="./out"):
    ...
    for epoch in range(num_epochs):
        for batch in train_dataloader:
            batch = {k: v.to(device) for k, v in batch.items()} # move it to gpu because gpu can't process large dataset at once, we need to iterate each batch instead
            # forward pass to get the prediction
            # batch is a dictionary containing key value pairs 
            # and the key will be passed in as the name arguments
            outputs= model(**batch)
            loss = outputs.loss 

            # once we get the loss, we need to calculate the gradient with back propogration
            loss.backward()
            # update the weights basd on the gradient using adam 
            optimizer.step()
        ...
        # backpropogation to get the gradient 
        # update the weights
        # calculate the loss
...
def create_augmented_dataloader(args, dataset):
    ...
    # randomly pick 5000 samples from the training dataset 
    # apply the transformation to these 5000 samples 
    # return these train_dataloader 
    train_full_dataset = dataset['train']
    sample = train_full_dataset.shuffle(seed = 33).select(range(5000))
    # what does load_from_cache_file mean? 
    transformed_sample = sample.map(custom_transform, load_from_cache_file=False)

    tokenized_original = train_full_dataset.map(tokenize_function, batched=True, load_from_cache_file=False)
    tokenized_transformed = transformed_sample.map(tokenize_function, batched=True, load_from_cache_file=False)
 
    # because pytorch expects labels, not label. 
    tokenized_original = tokenized_original.remove_columns(["text"])
    tokenized_original = tokenized_original.rename_column("label", "labels")
    tokenized_original.set_format("torch")

    tokenized_transformed = tokenized_transformed.remove_columns(["text"])
    tokenized_transformed = tokenized_transformed.rename_column("label", "labels")
    tokenized_transformed.set_format("torch")   
    # turn it into tensor 
    # Sets format to PyTorch tensors (so DataLoader can read them efficiently)
    augmented_train_dataset = concatenate_datasets([tokenized_original, tokenized_transformed])
...
if __name__ == "__main__":
    ...
    if args.debug_train:
        print("Debug mode: showing dataset preview only...\n")
    from datasets import load_dataset
    dataset = load_dataset("imdb")
    print(dataset)
    print("\nExample training sample:")
    print(dataset["train"][0])
    exit()  # stop execution before training starts
